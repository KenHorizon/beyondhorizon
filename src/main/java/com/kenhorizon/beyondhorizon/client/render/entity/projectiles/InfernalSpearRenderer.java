...
public class InfernalSpearRenderer extends AnimatedTextureRenderer<InfernalSpear> {
    ...
    public InfernalSpearRenderer(EntityRendererProvider.Context context) {
        super(context);
        this.model = new BaseSpearModel(context.bakeLayer(BHModelLayers.BASE_SPEAR));
    }

    @Override
    protected int getBlockLightLevel(InfernalSpear entity, BlockPos blockPos) {
        return 15;
    }

    @Override
    public void render(InfernalSpear entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        poseStack.pushPose();
    ...
    public String getTextureLocation() {
        return "textures/entity/projectiles/infernal_spear/infernal_spear";
    }
    ...
    private void renderTrail(InfernalSpear entityIn, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, float trailR, float trailG, float trailB, float trailA, int packedLightIn) {
        int sampleSize = 10;
