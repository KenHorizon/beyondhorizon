...
//            entity.setMotion(entity.getMotion().add(vector3d.normalize().scale(1.0)));
            ...
            this.dodgeVec = this.avoidTarget.getDeltaMovement().cross(new Vec3(0, 1, 0)).normalize().scale(1);
            Vec3 newPosLeft = this.entity.position().add(this.dodgeVec);
            Vec3 newPosRight = this.entity.position().add(this.dodgeVec.scale(-1));
            ...
            if (diffRight.lengthSqr() > diffLeft.lengthSqr()) {
                this.dodgeVec = this.dodgeVec.scale(-1);
            }
            ...
            if (vector3d == null) {
                this.path = null;
                return true;
            } else if (projectilePos.subtract(vector3d).lengthSqr() < projectilePos.subtract(this.entity.position()).lengthSqr()) {
                return false;
            } else {
