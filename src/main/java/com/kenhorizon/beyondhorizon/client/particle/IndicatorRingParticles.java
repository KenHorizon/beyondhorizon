...
public class IndicatorRingParticles extends TextureSheetParticle {
    ...
    public IndicatorRingParticles(ClientLevel level, double x, double y, double z,
                                  double motionX, double motionY, double motionZ,
                                  float yaw, float pitch, int duration,
                                  float r, float g, float b, float opacity, float size, Behavior behavior) {
        super(level, x, y, z);
        this.setSize(1, 1);
        this.size = size * 0.1f;
    ...
    @OnlyIn(Dist.CLIENT)
    public static final class Provider implements ParticleProvider<IndicatorRingParticleOptions> {
        ...
        @Override
        public Particle createParticle(IndicatorRingParticleOptions typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            IndicatorRingParticles particle = new IndicatorRingParticles(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, typeIn.getYaw(), typeIn.getPitch(), typeIn.getDuration(), typeIn.getR(), typeIn.getG(), typeIn.getB(), typeIn.getA(), typeIn.getScale(), typeIn.getBehavior());
