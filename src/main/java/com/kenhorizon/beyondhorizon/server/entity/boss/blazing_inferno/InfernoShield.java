    ...
    public void tick() {
        ...
        if (owner != null) {
            if (this.level().isClientSide()) {
                for (int i = 0; i < 2; i++) {
                    this.level().addParticle(new ParticleTrailOptions(0, 0, 20, 1.0F, 0.0F, 0.0F, 1.0F, 1.0F, true,  TrailParticles.Behavior.SHRINK, owner.position().add(0, owner.getBbHeight() * 0.5D, 0)), this.getRandomX(0.25D), this.getY() + (this.getBbHeight() / 2), this.getRandomZ(0.25D), 0, 0, 0);
                }
            }
