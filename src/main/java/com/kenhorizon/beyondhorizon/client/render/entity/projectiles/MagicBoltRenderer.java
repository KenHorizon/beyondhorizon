...
public class MagicBoltRenderer extends EntityRenderer<MagicBolt> {
    private static final ResourceLocation TEXTURE_RED = BeyondHorizon.resource("textures/entity/projectiles/magicbolt.png");
    ...
    @Override
    public void render(MagicBolt entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        poseStack.pushPose();
        ...
        if (entity.hasTrail()) {
            ...
            float ran = 0.04f;
            float r = 65/255F + this.random.nextFloat() * ran ;
            float g = 65/255F + this.random.nextFloat() * ran;
            float b = 112/255F + this.random.nextFloat() * ran * 1.5F;
            poseStack.pushPose();
            poseStack.translate(-x, -y, -z);
            renderTrail(entity, partialTicks, poseStack, buffer, r, g, b, 0.70F, packedLight);
            poseStack.popPose();
        }
        super.render(entity, yaw, partialTicks, poseStack, buffer, packedLight);
    }
    private void renderTrail(MagicBolt entityIn, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, float trailR, float trailG, float trailB, float trailA, int packedLightIn) {
        int sampleSize = 32;
        float trailHeight = 0.25F;
        float trailYRot = 0;
        float trailZRot = 0;
