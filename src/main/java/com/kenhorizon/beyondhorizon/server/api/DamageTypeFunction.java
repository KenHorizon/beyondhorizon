...
import com.kenhorizon.beyondhorizon.server.damagesource.DamageInfo;
...
public interface DamageTypeFunction {
    float calculate(float magnitude, float level, MobType mobType, DamageContext context, DamageSource source, LivingEntity attacker, LivingEntity target);
    DamageTypeFunction TARGET_CURRENT_HEALTH = ((magnitude, level, mobType, context, source, attacker, target) -> {
    ...
    DamageTypeFunction TARGET_MAX_HEALTH = ((magnitude, level, mobType, context, source, attacker, target) -> {
    ...
    DamageTypeFunction TARGET_MISSING_HEALTH = ((magnitude, level, mobType, context, source, attacker, target) -> {
        return DamageInfo.getMissingHealth(target, context, magnitude);
    });
    DamageTypeFunction USER_CURRENT_HEALTH = ((magnitude, level, mobType, context, source, attacker, target) -> {
        return DamageInfo.getCurrentHealth(attacker, context, magnitude);
    });

    DamageTypeFunction USER_MAX_HEALTH = ((magnitude, level, mobType, context, source, attacker, target) -> {
        return DamageInfo.getMaxHealth(attacker, context, magnitude);
    });

    DamageTypeFunction USER_MISSING_HEALTH = ((magnitude, level, mobType, context, source, attacker, target) -> {
        return DamageInfo.getMissingHealth(target, context, magnitude);
    });
    DamageTypeFunction BONUS_DAMAGE = ((magnitude, level, mobType, context, source, attacker, target) -> {
    ...
    DamageTypeFunction ARMORED_DAMAGE = ((magnitude, level, mobType, context, source, attacker, target) -> {
    ...
    DamageTypeFunction PERFECTION = ((magnitude, level, mobType, context, source, attacker, target) -> {
    ...
    DamageTypeFunction KINETIC_WEAPON = ((magnitude, level, mobType, context, source, attacker, target) -> {
