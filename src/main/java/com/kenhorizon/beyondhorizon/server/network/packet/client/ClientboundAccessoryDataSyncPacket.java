...
public record ClientboundAccessoryDataSyncPacket(CompoundTag nbt) {

    public ClientboundAccessoryDataSyncPacket(FriendlyByteBuf buf) {
        this(buf.readNbt());
    }
        ...
        context.enqueueWork(() -> {
            ...
            if (player != null) {
                AccessoryHelper.getInventory(player).ifPresent(handler -> {
                    handler.deserializeNBT(this.nbt);
                });
            }
        });
