package com.kenhorizon.beyondhorizon.server.level.entity.util;
...
public class ShockwaveUtils {
    ...
    public static List<LivingEntity> doRingShockwave(Entity entity, Vec3 center, double radius,
        ...
        for (int i = zFrom; i <= zTo; i++) {
            for (int j = xFrom; j <= xTo; j++) {
                ...
                Quaternionf rotator = Maths.rotation(axis, (float) (distance / radius) * 15F + level.random.nextFloat() * 10F - 5F, true);
                rotator.mul(Maths.XP.rotationDegrees(level.random.nextFloat() * 12F - 6F));
                rotator.mul(Maths.YP.rotationDegrees(level.random.nextFloat() * 40F - 20F));
                rotator.mul(Maths.ZP.rotationDegrees(level.random.nextFloat() * 12F - 6F));
    ...
    private static void trySpawnFallingBlock(ServerLevel level, BlockPos pos,
        ...
        BHFallingBlocks entity;
        if (rot != null) {
            entity = new BHFallingBlocks(level, block, rot, lifeTime, bounce);
            entity.setPos(pos.getX() + 0.5, pos.getY() + 1, pos.getZ() + 0.5);
        } else if (useRandomOffset) {
            entity = new BHFallingBlocks(level, block, (float) (0.32 + bounce * 0.2));
            entity.setPos(pos.getX() + 0.5, pos.getY() + 1, pos.getZ() + 0.5);
        } else {
            entity = new BHFallingBlocks(level, pos.getX() + 0.5, pos.getY() + 1, pos.getZ() + 0.5, block, lifeTime);
