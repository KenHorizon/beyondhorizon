...
public class ApplyEffectAccessory extends AccessoryPassiveSkill {
    ...
    protected MutableComponent makeTooltip(ItemStack itemStack) {
        if (this.effectOccured == 1.0D) {
            return Component.translatable(String.format("%s.desc", this.getDescriptionId()), this.getSeconds());
        } else {
            return Component.translatable(String.format("%s.desc", this.getDescriptionId()), Maths.decimal(this.getEffectOccured()), getSeconds());
        }
    }
